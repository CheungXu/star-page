...
**最终选型**：方案三（motion）为主，方案二（View Transitions）/ 方案一（纯 CSS）依次为
运行时降级，已集成进生产。决策与实现见 `wiki/frontend-home-workspace-transition.md` 与
`doc/20260529/frontend-transition-animation-plan.md`。原型目录使命完成，可按需归档或清理。
