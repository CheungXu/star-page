...
- `frontend-home-workspace-transition.md`：首页 ↔ 工作区两态切换的平滑衔接过渡——用命令式过渡协调器解耦「改状态」与「怎么动画」，排出「motion 主 → View Transitions 备 → 纯 CSS 兜底 → reduced-motion 直接切换」的运行时三级降级链；含 React 集成 5 个关键点（稳定舞台容器 + key、`flushSync` 同步提交、动态 import 降级、内联样式防 React 覆盖、首屏恢复不走过渡）与 FLIP 飞行 / 文字变气泡 / stagger 入场的命令式技巧。
