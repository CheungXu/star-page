...
- `20260524/mvp-main-flow-implementation-record.md`：MVP 主流程实现记录，包含数据库迁移、LLM 生成、OSS 存储、页面预览、历史会话、systemd 常驻运行等实施状态。

## 使用约定

- `doc/` 记录本项目阶段性工作过程、实施状态和问题处理记录。
- 可跨项目复用的通用经验沉淀到 `wiki/`，例如 systemd 常驻部署、生成页预览方案、LLM Provider 抽象原则。
