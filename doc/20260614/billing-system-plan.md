...
- 匿名模型限制（控成本）：匿名只能选次优模型（如允许 `deepseek-v4-flash`、`qwen-plus`、`doubao`），高价旗舰（`qwen`(max)、`deepseek-v4-pro`、`glm-5.1`、`kimi-k2.6`、`minimax-m3` 等）**仍然可见但置灰**——点击时轻量提示"注册后可用"，用以吸引注册而非隐藏；**单次最多生效 2 个模型**，勾第 3 个时轻量提示需注册；`max_tokens` 不变。允许集合走配置，可维护。
