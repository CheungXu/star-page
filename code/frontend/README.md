...
- **运行时三级降级链**：`runStageTransition()` 依次判断——主用 motion 库（FLIP 输入卡
  飞行 + 文字上浮成气泡 + 内容 stagger 入场）；motion 依赖加载失败时降级到原生
  View Transitions API；浏览器不支持 `startViewTransition` 时再降级到纯 CSS 交叉过渡；
  `prefers-reduced-motion` 则直接切换、无动画。
...
- **稳定舞台**：两态外层包 `<div className="app-stage" ref={stageRef}>`，两态 `<main>`
  各带 `key`（`hero` / `workspace`）。协调器用 `flushSync` 同步提交状态切换，以便在切换
  前后测量位置 / 让浏览器捕获新 DOM。
- **motion 动态加载**：`import("motion")` 在 `useEffect` 里异步加载并存入 `motionRef`，
  失败保持为空自动降级；首屏从 localStorage 恢复非首页态时**不**走过渡（无「从首页来」
  的语境）。
- **动画用内联样式**：动画进行中组件可能因其它 `setState` 重渲染而覆盖 className，所以
  入场/离场全走内联 `style` 或 WAAPI；需要逐项 stagger 入场的容器打 `data-anim-stagger`。
- **CSS 侧**：`globals.css` 末尾只承载 View Transitions 降级路径的共享元素命名
  （`html.vt-switching .prompt-card` 等作用域化）与 `::view-transition-*` 关键帧；motion
  与纯 CSS 路径完全走内联样式。

跨项目可复用的设计与实现经验沉淀在 `wiki/frontend-home-workspace-transition.md`；选型用的
多端口原型在 `script/preview-transition/`。
