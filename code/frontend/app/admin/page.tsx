...
type Tab = "overview" | "transactions" | "ledger" | "users" | "markups" | "bill";
  ...
  return (
    <main className="admin-page">
      ...
      <nav className="admin-tabs">
        {(["overview", "transactions", "ledger", "users", "markups", "bill"] as Tab[]).map((key) => (
...
function OverviewView({ data, onRefresh }: { data: Overview; onRefresh: () => Promise<void> }) {
  ...
  return (
    <>
      ...
      <section className="admin-section-block">
        <h3 className="admin-section-title">营业利润<span>含赠送毛利再扣除服务器等基础设施成本</span></h3>
        <div className="admin-cards">
          ...
          <div className="admin-card is-total" key="op">
            ...
            <span className="admin-card-hint">综合毛利 − 基础设施成本</span>
          </div>
        </div>
      </section>
